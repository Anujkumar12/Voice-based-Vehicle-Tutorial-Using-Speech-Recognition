...
void read_codebook2();
long double abs12(long double);
void compute_max_sample2(char*);
long double compute_energy2(int);
void dcShift2();
void normalize2();
void compute_ri2();
void compute_ai2();
void compute_ci2();
void compute_raised_ci2();
void compute_observation_symbol2();
...
int recording_test()
{
	...
	printf("*******recording digit *****************");
		...
		prob=0;
		x=0;
		strcpy(modelfilename11,"models\\iteration_3\\");
		strcat(modelfilename11,userName);
		strcat(modelfilename11, "_");
		result=test(3);
		/*if(prob<0.01)
		{	
			strcpy(modelfilename11,"models\\iteration_2\\");
			strcat(modelfilename11,userName);
			strcat(modelfilename11, "_");
			result=test(2);
		...
		if(prob<0.01)
		{	
			strcpy(modelfilename11,"models\\iteration_1\\");
			strcat(modelfilename11,userName);
			strcat(modelfilename11, "_");
			result=test(1);
...
void load_file()			//This function used to load recording file for testing
{
	...
	avg_energy=0;
	avg_sample=0;
	compute_max_sample2("input_file.txt");		


	FILE *fp=fopen("input_file.txt","r");		//file opening
...
void read_codebook2()  //This function is used to read codebook from file
{
...
long double abs12(long double t)
{
	if(t<0)
		return -t;
	else
...
void compute_max_sample2(char filename[50])			//This function compute the maximum sample for normalization
{
	int count=0,f=0,max_index=0;
	long double x=0,max=0,sum=0,sum1=0,max_energy=0;
	FILE *fp1=fopen(filename,"r");

	FILE *fp=fopen("input_file01.txt","w");
	if(fp1==NULL)
	{
...
long double compute_energy2(int x)		//This function is used to compute energy of a frame
{
	long double sum=0;
...
void normalize2()		// This function is used to compute normalized values for each frame.
{
...
void compute_ri2()				//This function compute the ri's for input frame
{
	long double sum=0;
...
void compute_ai2()			//This function computes ai's for input frame
{
	int i=0,j=0;
	int p=12;
	long double sum=0;
...
void compute_ci2()				//This function compute ci's for input frame
{	
	int p=12;
...
void compute_raised_ci2()				//this function is used to apply RAISED SINE WINDOW on ci's.
{	
	int i=0;
...
void compute_observation_symbol2()			//This function used to compute observation sequence for a input speech signal
{
	int minindex=0;
